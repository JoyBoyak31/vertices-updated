import React, { useState, useEffect } from 'react';
import styles from './Tokenomics.module.css';
import logo from '../assets/logo/logo.png';
const Tokenomics = () => {
  const [isVisible, setIsVisible] = useState(false);

  useEffect(() => {
    setIsVisible(true);
  }, []);

  // Calculate positions in a perfect circle with proper spacing
  const calculatePosition = (index, total) => {
    // Different radius for different screen sizes
    const getRadius = () => {
      if (window.innerWidth >= 1400) return 480;
      if (window.innerWidth >= 1200) return 420;
      if (window.innerWidth >= 1024) return 380;
      return 340;
    };
    
    const radius = getRadius();
    const angle = (index * 360) / total - 90; // Start from top (-90 degrees)
    const radian = (angle * Math.PI) / 180;
    const x = Math.cos(radian) * radius;
    const y = Math.sin(radian) * radius;
    
    return {
      left: `calc(50% + ${x}px - 110px)`, // 110px = half card width
      top: `calc(50% + ${y}px - 70px)`    // 70px = half card height
    };
  };

  const tokenData = [
    { 
      label: ' Community Rewards & Staking', 
      percentage: 20, 
      color: '#ff00ff',
      description: 'Long-term staking rewards & adoption incentives'
    },
    { 
      label: ' Ecosystem Growth Fund', 
      percentage: 18, 
      color: '#00ffff',
      description: 'Dev grants, cross-chain expansion, dApp partners'
    },
    { 
      label: 'Team & Founders (4-year vesting, 1-year cliff)', 
      percentage: 12, 
      color: '#ff6b6b',
      description: '4-year vesting with 1-year cliff'
    },
    { 
      label: 'Treasury & Operations', 
      percentage: 10, 
      color: '#4ecdc4',
      description: 'Governance proposals & operational expenses'
    },
    { 
      label: 'Airdrop Campaigns', 
      percentage: 10, 
      color: '#45b7d1',
      description: 'Transparent user onboarding & milestone-based release'
    },
    { 
      label: 'Strategic Partners', 
      percentage: 10, 
      color: '#96ceb4',
      description: 'Aligned partners in AI, logistics, retail'
    },
    { 
      label: 'Merchant Onboarding & Readiness', 
      percentage: 10, 
      color: '#ffa726',
      description: 'Incentivize merchants & support onboarding'
    },
    { 
      label: 'Advisors & Experts', 
      percentage: 5, 
      color: '#ab47bc',
      description: 'Ensures smooth trading & price stability'
    },
    { 
      label: '  Listing Liquidity', 
      percentage: 5, 
      color: '#66bb6a',
      description: 'Domain leaders in compliance, AI, logistics'
    }
  ];

  const InfoCard = ({ data, index, total }) => {
    const position = calculatePosition(index, total);
    
    return (
      <div 
        className={styles.infoCard}
        style={{
          ...position,
          animationDelay: `${index * 0.15}s`
        }}
      >
        <div className={styles.cardContent}>
          <div className={styles.cardHeader}>
            <div 
              className={styles.colorIndicator}
              style={{ backgroundColor: data.color }}
            ></div>
            <span className={styles.percentage}>{data.percentage}%</span>
          </div>
          <div className={styles.cardTitle}>{data.label}</div>
          <div className={styles.cardDescription}>{data.description}</div>
          <div 
            className={styles.cardLine} 
            style={{ backgroundColor: data.color }}
          ></div>
        </div>
      </div>
    );
  };

  return (
    <div className={`${styles.tokenomicsWrapper} ${isVisible ? styles.visible : ''}`}>
      <div className={styles.gridBackground}></div>
      
      <h1 className={styles.title}>TOKENOMICS</h1>
      
      <div className={styles.tokenomicsContainer}>
        {/* Center Circle */}
        <div className={styles.centerCircle}>
          <div className={styles.innerCircle}>
            <div className={styles.tokenSymbol}>
              <span className={styles.tokenIcon}>◊</span>
              <span className={styles.tokenName}>VTXN</span>
            </div>
          </div>
        </div>
        
        {/* Token Distribution Cards */}
        {tokenData.map((data, index) => (
          <InfoCard 
            key={index} 
            data={data} 
            index={index} 
            total={tokenData.length} 
          />
        ))}
        
        {/* Total Supply */}
        <div className={styles.totalSupply}>
          <div className={styles.supplyLabel}>Total Supply</div>
          <div className={styles.supplyValue}>2,100,000,000 VTXN</div>
        </div>
      </div>

      {/* Mobile Cards Grid */}
      <div className={styles.mobileCardsGrid}>
        {tokenData.map((data, index) => (
          <div key={index} className={styles.mobileCard}>
            <div className={styles.mobileCardHeader}>
              <div 
                className={styles.mobileColorIndicator}
                style={{ backgroundColor: data.color }}
              ></div>
              <span className={styles.mobilePercentage}>{data.percentage}%</span>
            </div>
            <div className={styles.mobileCardTitle}>{data.label}</div>
            <div className={styles.mobileCardDescription}>{data.description}</div>
          </div>
        ))}
      </div>
    </div>
  );
};

export default Tokenomics;